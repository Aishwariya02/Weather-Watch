...
def open_news():
    ...
    label_frame = LabelFrame(News_window, text="Current News", width=2, height=5, bd=3,bg="Lightcyan2", font=("Comic Sans MS", 12))
    label_frame.pack()
...
def open_forecast():
    win = Toplevel()
    win.geometry("1280x660")
    win.title("Weather app")
    f = ("poppins", 12, "bold")
    t = ("poppins", 15, "bold")

    bg = PhotoImage(file='landscape.png')
    bg_lable = Label(win, image=bg)
    ...
    sun_image = PhotoImage(file='2275.png')
    sun_lable = Label(win, image=sun_image, bg='white')
    sun_lable.place(x=750, y=100)
...
def open_notes():
    ...
    Display.geometry("600x360")
    file=open("Notes.txt",'r')
    content=file.readlines()
    file.close()
    for line in content:
        Label(Display,text=line,font=("Comic Sans MS",12)).pack()
    Button(Display,text="Back",font=("Comic Sans MS",12), width=5, command=delete_display).place(x=20,y=280)
    Button(Display,text="Clear Notes",font=("Comic Sans MS",12),width=12, command=clear_notes).place(x=460,y=280)
...
def clear_notes():
        ...
        Button(notes_clear_window, text="OK", width=6, font=("Comic Sans MS",10), command=lambda:[delete_notes_clear_window(),delete_display()]).pack()
